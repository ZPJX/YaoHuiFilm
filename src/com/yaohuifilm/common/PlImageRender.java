 ...
 * PlImageRender.
 */
public class PlImageRender extends Render {
	...
	public PlImageRender(File file) {
		this.file = file;
	}
	
	public PlImageRender(String fileName) {
		this.fileName = fileName;
	}
	
	static void init(String fileDownloadPath, ServletContext servletContext) {
		PlImageRender.fileDownloadPath = fileDownloadPath;
		PlImageRender.servletContext = servletContext;
	...
	public void render() {
		...
		try {
			response.addHeader("Content-disposition", "inline; filename=" + new String(file.getName().getBytes("GBK"), "ISO8859-1"));
